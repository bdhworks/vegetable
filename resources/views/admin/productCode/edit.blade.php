...
@section('content')
<div class="product-code-edit-page">
    <!-- Page Header -->
    ...
    <div class="form-container">
        <form method="POST" action="{{ route('productCode.update', $productCode) }}" id="editProductCodeForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="content-card">
                        <div class="card-header-custom">
                            <div class="header-left">
                                ...
                                <p class="card-subtitle-custom">Cập nhật thông tin chi tiết cho mã sản phẩm</p>
                            </div>
                            <div class="header-right">
                        ...
                        <div class="card-body-custom">
                            ...
                            <div class="form-group-modern">
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('name', $productCode->name) }}"
                                       required>
                            ...
                            <div class="form-group-modern">
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('title', $productCode->title) }}"
                                       required>
                                @error('title')
                                    <div class="error-message">
                                        <i class="ti ti-alert-circle"></i>
                            ...
                            <div class="form-group-modern">
                                ...
                                <textarea class="form-control-modern ckeditor" 
                                          ...
                                          id="desc"
                                          rows="6"
                                          placeholder="Nhập mô tả cho sản phẩm...">{{ old('desc', $productCode->desc) }}</textarea>
                                @error('desc')
                                    <div class="error-message">
                                        <i class="ti ti-alert-circle"></i>
                                        <span>{{ $message }}</span>
                            ...
                            <div class="history-timeline">
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        <h6>Tạo mới</h6>
                                        <p>{{ $productCode->created_at->format('d/m/Y H:i') }}</p>
                                        <span class="timeline-time">{{ $productCode->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <p>{{ $productCode->updated_at->format('d/m/Y H:i') }}</p>
                                        <span class="timeline-time">{{ $productCode->updated_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body-custom">
                            ...
                            <div class="form-group-modern">
                                ...
                                <select name="status" class="form-select-modern" required>
                                    ...
                                    <option value="1" {{ old('status', $productCode->status) == '1' ? 'selected' : '' }}>
                                        Hiển thị
                                    </option>
                                    <option value="0" {{ old('status', $productCode->status) == '0' ? 'selected' : '' }}>
                                        Ẩn
                                    </option>
                                </select>
                                @error('status')
                            ...
                            <div class="info-box">
                                ...
                                <div class="info-content">
                                    <h6>Thông tin</h6>
                                    <ul>
                                        <li><strong>ID:</strong> {{ $productCode->id }}</li>
                                        <li><strong>Mã:</strong> {{ $productCode->name }}</li>
                                        <li><strong>Tạo:</strong> {{ $productCode->created_at->format('d/m/Y') }}</li>
                                        <li><strong>Cập nhật:</strong> {{ $productCode->updated_at->format('d/m/Y') }}</li>
                                    </ul>
                                </div>
                            </div>
                            ...
                            <div class="form-actions-sidebar">
                                <button type="submit" class="btn btn-update">
                                    <i class="ti ti-check"></i>
                                    <span>Cập nhật mã sản phẩm</span>
                                </button>
                        ...
                        <div class="card-body-custom">
                            <div class="preview-box">
                                <div class="preview-code-badge">
                                    ...
                                    <span id="preview-code">{{ $productCode->name }}</span>
                                </div>
                                <h6 class="preview-title" id="preview-title">{{ $productCode->title }}</h6>
...
<form id="deleteForm" action="{{ route('productCode.destroy', ['productCode_id' => $productCode->id]) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
</form>

<style>
/* Product Code Edit Page Styles */
.product-code-edit-page {
    padding: 1.5rem;
    background: #f5f7fa;
    min-height: 100vh;
}
...
function confirmDelete() {
    if (confirm('Bạn có chắc chắn muốn xóa mã sản phẩm này? Hành động này không thể hoàn tác!')) {
        document.getElementById('deleteForm').submit();
    }
}

// Form validation
document.getElementById('editProductCodeForm')?.addEventListener('submit', function(e) {
